# This program is going to take the host, user, password from the Operating System user's Environment variable.
# first we have to defined the user's environment variables using the search "edit environment variable" and then creating
# each one environment varible for DB_USER and DB_PASSWORD, Then need to import the 'os' library. the 'os' library
# contains a dictionary type variable calle 'environ' that contains all the User's environment variables that we have
# defined by using the os.environ.get('DB_USER') or os.environ.get('DB_PASSWORD')
