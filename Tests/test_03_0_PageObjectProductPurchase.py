...
class TestEndToEndShop(BaseClassUtilities):
    ...
    def test_select_product_and_purchase(self):   # setup was eleminated because with 'self' we have now access to
        ...
        print("wk_product_to_select", wk_product_to_select)
        ...
        checkout_page_object = home_page_object.get_shop_link()

        # No need to instantiate a checkoutPage object from the CheckoutPage class:
        # ##   --> checkout_page_object = CheckoutPage(self.driver)
        ...
        if wk_product_found:
            ...
            # click on the 'Final Checkout' button and instantiate object for the next page displayed:
